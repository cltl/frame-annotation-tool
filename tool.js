...
function getLatestExternalReference(references) {
    ...
    if (!Array.isArray(references)) {
        ...
        for (var i in references) {
            ...
            if (most_recent) {
                if (
                    moreRecent(most_recent["attr"]["timestamp"], reference_time)
                ) {
        ...
        if (token["subtoken"] != undefined) {
            ...
            for (var j in subtokens) {
                if ("#cdata" in subtokens[j]) {
                    token_subs[subtokens[j]["attr"]["id"]] = {
                        text: subtokens[j]["#cdata"],
                    };
                } else {
                    token_subs[subtokens[j]["attr"]["id"]] = {
                        text: subtokens[j]["#text"],
                    };
                }
            }
        }
    ...
    for (var i in term_layer) {
        ...
        // Term is part compound
            ...
            for (var j in sub_terms) {
                ...
                var target_subtoken_id =
                    sub_term["span"]["target"]["attr"]["id"];
                var target_sub_token = target_token["sub"][target_subtoken_id];

                var sub_term_data = {
                    text: target_sub_token["text"],
...
function readSRLLayer(srl_layer, typicality) {
    ...
    for (var i in srl_layer) {
        ...
        if (predicate_stat !== "deprecated") {
            ...
            if (!Array.isArray(predicate_span))
...
function addMultiwordEntry(json_data, multiword_data) {
    json_data["NAF"] = createLayerIfNotExists(
        json_data["NAF"],
        "multiwords",
        "mw"
    );
    ...
    multiword_data["mcn_type"] =
        multiword_data["mcn_type"] == 1 ? "phrasal" : "idiom";
    ...
    if (pos == "") {
        delete multiword_entry["attr"]["pos"];
    }
...
function deprecateMultiwordEntry(json_data, target_id) {
    ...
    for (var i in term_layer) {
        ...
        if (
            "component_of" in term["attr"] &&
            term["attr"]["component_of"] == target_id
        ) {
...
function removeCompoundEntry(json_data, target_id) {
    ...
    for (var i in term_layer) {
        ...
        if (term_id == target_id) {
            ...
            target_id =
                term_layer[i]["span"]["target"]["attr"]["id"].toString();
...
function addPredicateEntry(json_data, predicate_data, session_id) {
    ...
    var predicate_entry = {
        attr: { id: predicate_id, status: "manual" },
        span: { target: [] },
    ...
    var reference_data = {
        reference: predicate_data["frame"],
...
function addCoreferenceEntry(json_data, coreference_data, session_id) {
    ...
    var reference_data = {
        reference: coreference_data["reference"],
        resource: "http://www.wikidata.org",
        timestamp: new Date().toNAFUTCString(),
        source: session_id,
        reftype: coreference_data["type"],
    };
    correference_entry = addExternalReferences(
        correference_entry,
        reference_data
    );
...
function handleMarkableCorrection(json_data, task_data) {
    ...
    if (task_data.mcn_task == 1) {
        if (task_data.mcn_type == 1 || task_data.mcn_type == 2) {
            ...
            return updateMultiwordTerms(
                json_data,
                mw_id,
                task_data["target_ids"]
            );
...
function handleFrameAnnotation(json_data, task_data, session_id) {
    if (task_data.fan_task == 1) {
        ...
        if (task_data["lu"] == "") {
            task_data["lu"] = dynamicLexicalLookup(
                task_data["lem"],
                task_data["pos"],
                task_data["frame"]
            );
        ...
        for (var i in srl_layer) {
            ...
            var target_index =
                task_data["target_ids"].indexOf(predicate_target_id);

            if (target_index > -1) {
                task_data["target_ids"].splice(target_index, 1);
                var reference_data = {
                    reference: task_data["frame"],
                    resource: "http://premon.fbk.eu/premon/fn17",
                    timestamp: timestamp,
                    source: session_id,
                    lu_url: task_data["lu"],
...
var getFrameElements = function (frame_id) {
    ...
    for (var i = 0; i < frame_elements.length; i++) {
        ...
        var element = {
            label: element_label,
            value: element_premon,
            definition: element_definition,
            framenet: frame_framenet,
        };

        // Add current frame element to correct resulting list
        if (element_type == "Core") core_elements.push(element);
        else if (element_type == "Peripheral")
            peripheral_elements.push(element);
        else if (element_type == "Extra-Thematic")
            extra_thematic_elements.push(element);
        // else if (element_type == "Core-Unexpressed") core_unexpressed_elements.push(element);
        else if (element_type == "Core-Unexpressed")
            core_elements.push(element);
        else continue;
    }
    ...
    return {
        Core: core_elements,
        Peripheral: peripheral_elements,
        "Extra-Thematic": extra_thematic_elements,
    };
};
        ...
        if (lp["attr"]["layer"] == "srl") {
            ...
            for (var p = 0; p < processes.length; p++) {
                ...
                if (processId == sessionId) {
                    ...
                    // we don't have this session yet -> store it
                    var currentProcess = {
                        attr: {
                            beginTimestamp: loginTime,
                            endTimestamp: actionTime,
                            name: "annotator_" + annotator,
                            version: "guidelines_" + GUIDELINESVERSION,
                            id: sessionId,
                        },
                    };
    ...
    if (tid == 5) {
        ...
        loadNAFFile(lan + "/" + doc, "", false, true, function (json_data) {
            if (tid == 1) {
                ...
                json_data = handleFrameAnnotation(
                    json_data,
                    tda,
                    req.sessionID
                );
                json_data = saveSessionInfo(
                    json_data,
                    req.sessionID,
                    user,
                    login_time
                ...
                json_data = handleFrameElementAnnotation(
                    json_data,
                    tda,
                    req.sessionID
                );
